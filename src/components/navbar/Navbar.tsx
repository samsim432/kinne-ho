...
import { Search, ChevronDown, ChevronRight, HelpCircle, Heart, MessageSquare, Plus, LogOut, Camera } from 'lucide-react';
import { KINNE_HO_CATEGORIES, type MainCategory } from '../../data/vintedCategories';
  ...
  return (
    <header className="w-full bg-white border-b border-gray-200 sticky top-0 z-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        {/* Search Bar with Catalogue Dropdown */}
        <form onSubmit={handleSearch} className="flex-1 max-w-2xl flex items-center bg-[#f3f4f6] hover:bg-[#eaecee] focus-within:bg-white focus-within:ring-2 focus-within:ring-[#1b7a53] rounded-xl transition-all">
          ...
          <div className="flex items-center flex-1 px-3 py-2">
            ...
            <input
              type="text"
              placeholder="Search items, categories, brands, or cities..."
              value={searchQuery}
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          {!user ? (
            <>
              <Link
                to="/auth?mode=select"
                className="px-3.5 py-2 text-xs font-bold text-[#1b7a53] border border-[#1b7a53] rounded-xl hover:bg-[#1b7a53]/5 transition-colors cursor-pointer"
              >
                Sign up | Log in
              </Link>
              ...
              <Link
                ...
                className="px-4 py-2 text-xs font-bold text-white bg-[#1b7a53] hover:bg-[#156343] rounded-xl transition-colors cursor-pointer shadow-xs"
              >
                Sell now
              </Link>
            ...
            <Link
  to="/safety"
  className="text-gray-400 hover:text-[#1b7a53] p-1.5 transition-colors cursor-pointer"
  title="Help Center"
>
  <HelpCircle className="w-5 h-5 stroke-[1.8]" />
            ...
            <>
              <Link to="/favorites" className="p-2 text-gray-600 hover:text-gray-900 relative" title="Saved Wishlist">
                <Heart className="w-5 h-5" />
                {favorites.length > 0 && (
                  <span className="absolute top-1 right-1 w-2 h-2 bg-[#1b7a53] rounded-full" />
                )}
              </Link>

              <Link to="/messages" className="p-2 text-gray-600 hover:text-gray-900" title="Messages">
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1.5 p-1 rounded-full hover:bg-gray-100 cursor-pointer"
                >
                  <div className="w-8 h-8 rounded-full bg-emerald-100 text-[#1b7a53] flex items-center justify-center text-xs font-bold border border-emerald-200">
                ...
                {isUserMenuOpen && (
                  <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-200 rounded-2xl shadow-xl py-2 z-50 text-xs">
                    ...
                    <Link to="/my-listings" onClick={() => setIsUserMenuOpen(false)} className="block px-4 py-2 hover:bg-gray-50 text-gray-700">
                      My Listings
                    </Link>
                    <Link to="/wallet" onClick={() => setIsUserMenuOpen(false)} className="block px-4 py-2 hover:bg-gray-50 text-gray-700">
                      Wallet & Escrow
                    </Link>
                    <Link to="/admin" onClick={() => setIsUserMenuOpen(false)} className="block px-4 py-2 text-red-600 hover:bg-red-50 font-bold">
                      Admin Arbiter
                    </Link>
                    <button
                      onClick={() => {
                      ...
                      className="w-full text-left px-4 py-2 text-red-600 hover:bg-red-50 flex items-center gap-1.5 font-semibold cursor-pointer"
                    >
              ...
              <Link
                to="/sell"
                className="px-4 py-2 text-xs font-bold text-white bg-[#1b7a53] hover:bg-[#156343] rounded-xl transition-colors flex items-center gap-1 shadow-xs"
              >
